	...
	try {
		const stockData = await Promise.all(
			tickersArr.map(async (ticker) => {
				const url = `https://polygon-api-worker.ishmamhossain4.workers.dev/?ticker=${ticker}&startDate=${dates.startDate}&endDate=${dates.endDate}`;
...
async function fetchReport(data) {
	...
	try {
		const url = 'https://openai-api-worker.ishmamhossain4.workers.dev/';
